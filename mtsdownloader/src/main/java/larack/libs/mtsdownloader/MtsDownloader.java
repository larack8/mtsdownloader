...
import larack.libs.mtsdownloader.function.DownloadService;
...
public class MtsDownloader {
    ...
    private MtsDownloader() {
        mDownloadHelper = new DownloadHelper();
    }

    public static MtsDownloader getInstance() {
        return new MtsDownloader();
    }
     ...
     * @return mtsdownloader
     */
    public MtsDownloader context(Context context) {
        this.mContext = context;
        return this;
    }

    public MtsDownloader defaultSavePath(String savePath) {
        mDownloadHelper.setDefaultSavePath(savePath);
        return this;
    }

    public MtsDownloader retrofit(Retrofit retrofit) {
        mDownloadHelper.setRetrofit(retrofit);
        return this;
    }

    public MtsDownloader maxThread(int max) {
        mDownloadHelper.setMaxThreads(max);
        return this;
    }

    public MtsDownloader maxRetryCount(int max) {
        mDownloadHelper.setMaxRetryCount(max);
        return this;
    }

    public MtsDownloader maxDownloadNumber(int max) {
        this.MAX_DOWNLOAD_NUMBER = max;
        return this;
    }

    public MtsDownloader autoInstall(boolean flag) {
    ...
    public Observable<DownloadEvent> receiveDownloadStatus(final String url) {
        return Single.create(new SingleOnSubscribe<Object>() {
            ...
            public ObservableSource<? extends DownloadEvent> apply(Object o) throws Exception {
                return mDownloadService.processor(MtsDownloader.this, url).toObservable();
            }
    ...
    public Observable<?> deleteServiceDownload(final String url, final boolean deleteFile) {
        return createGeneralObservable(new GeneralObservableCallback() {
            @Override
            public void call() {
                mDownloadService.deleteDownload(url, deleteFile, MtsDownloader.this);
            }
        });
    }
    ...
    private void addDownloadTask(@NonNull String url, @NonNull String saveName,
        ...
        mDownloadService.addDownloadMission(
                new DownloadMission.Builder()
                        .setmtsdownloader(MtsDownloader.this)
                        .setUrl(url)
